...
# plot by treatment 
fit1 <- survfit(surv_object ~ treatment, data = raw)
...
apo <- raw %>%
  filter(apo_sym == "APO")
surv_object2 <- Surv(apo$days_alive, apo$survival_10.10.23)
fit3 <- survfit(surv_object2 ~ treatment, data = apo)
plot_apo <- ggsurvplot(fit3, data = apo, pval = TRUE, legend = "bottom", legend.title = "Treatment", legend.labs = c("Control", "Deep", "Shade"))
plot_apo <- plot_apo + xlab("Days, APO ONLY")
plot_apo
